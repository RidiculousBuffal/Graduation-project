...
class MyWeb3Client:
    ...
    def __init__(self):
        ...
        with open("OperationLogger.json", 'r') as f:
            contract_data = json.load(f)
            self.CONTRACT_ABI = contract_data['abi']
        self.contract = self.w3.eth.contract(address=self.CONTRACT_ADDRESS, abi=self.CONTRACT_ABI)

    def syncToBlockChain(self, AuditLogDTO: AuditLogDTO):
        try:
            txn = self.contract.functions.logOperation(AuditLogDTO.action.model_dump_json()).build_transaction({
            ...
            log_entry = AuditLog(user_id=AuditLogDTO.action.userId,
                                 action=AuditLogDTO.action.model_dump(),
